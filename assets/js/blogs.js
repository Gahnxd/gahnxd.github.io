...
async function loadBlogsList() {
    try {
        ...
        if (Object.keys(mdxFilesData).length === 0) {
            blogsGrid.innerHTML = '<p class="no-blogs">No blogs available yet. Check back soon!</p>';
            return;
        }
...
async function loadBlogContent(slug) {
    try {
        ...
        if (window.Prism) {
            // Use setTimeout to ensure DOM is fully updated before highlighting
            setTimeout(() => {
                Prism.highlightAll();
            }, 0);
        }
        ...
        if (window.MathJax && typeof window.MathJax.typeset === 'function') {
            // Use setTimeout to ensure DOM is fully updated before MathJax processing
            setTimeout(() => {
